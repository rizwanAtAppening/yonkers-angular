  ...
  getCurrenrUser() {
    this.authService.getUserInfo().subscribe((currentUser) => {
      ...
      if (this.currentUserInfo) {
        ...
        this.profileForm.controls.mobile_number.setValue(
          this.currentUserInfo.mobile_number
            ? this.currentUserInfo.mobile_number
            : null
        );
        this.profileForm.controls.phone_number.setValue(
        ...
        this.profileForm.controls.job_title.setValue(
          this.currentUserInfo.job_title
        );
        if (this.currentUserInfo.state) {
  ...
  phoneNumberFormate(value: string) {
    ...
    if (value == "phone") {
      ...
      if (this.profileForm.value.phone_number.length === 3) {
        this.profileForm.controls.phone_number.setValue(
          this.profileForm.value.phone_number.concat(autoFillValue)
        );
      }
      ...
      if (this.profileForm.value.mobile_number === null) {
        this.profileForm.controls.mobile_number.setValue(
          this.profileForm.value.mobile_number
        );
      }
      if (this.profileForm.value.mobile_number.length === 3) {
        this.profileForm.controls.mobile_number.setValue(
          this.profileForm.value.mobile_number.concat(autoFillValue)
        );
      }
