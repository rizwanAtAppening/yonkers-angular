...
export class AddPermitTabSectionComponent implements OnInit {
  ...
  getApplication() {
    ...
    this.addressId = this.addressId
      ? this.addressId
      : this.application.address_id;
    this.selectedValue = this.selectedValue
      ? this.selectedValue
      : this.application.address;
    if (this.application.applicant_details) {
      this.applicationState = Number(
        this.application.applicant_details.applicant_state
      );
    }
    if (this.application.contractor_details) {
      this.contractorDetails = Number(
        this.application.contractor_details.contractor_state
      );
    }
    ...
    if (
      this.application.upload_detail &&
      this.application.upload_detail.length > 0
    ) {
      this.allImage = [];
      if (this.application.upload_detail) {
        ...
        this.application[
          "drawings"
        ] = `${this.imageBasePath}${this.application.upload_detail[0].name}`;
        if (this.application.upload_detail.length > 1) {
          this.application[
            "CertificateofInsurance"
          ] = `${this.imageBasePath}${this.application.upload_detail[1].name}`;
        }
      }
  ...
  get duplimesterCon() {
    return this.duplimesterForm.controls;
  }
  ...
  addLicense() {
    ...
    formData.append("name", this.licenseFile);
    formData.append("type", this.addInsuranseForm.value.type);
  ...
  get licenseCon() {
    return this.addInsuranseForm.controls;
  }
  ...
  whereFormContrl() {
    this.whereForm = this.formBuilder.group({
      ...
      // street_two: [''],
      // address_join: [''],
      addlocation: this.formBuilder.array([this.addLocationFormGroup()]),
    });
  }
  ...
  addPermitApplication(formGroup, nextTab) {
    ...
    if (formGroup == "whatForm" || this.currentTab == "what") {
      ...
      this.currentTab == "contrator"
    ) {
      if (
        this.application.role == 2 ||
        (this.application.role == 1 && this.application.type != 4) ||
        (this.application.role == 3 && this.application.type != 4)
      ) {
      ...
      this.projectDetailsForm.value.purpose = Number(
        this.projectDetailsForm.value.purpose
      );
      this.projectDetailsForm.value.pavement_type = Number(
        this.projectDetailsForm.value.pavement_type
      );
      this.projectDetailsForm.value.traffic_control = Number(
        this.projectDetailsForm.value.traffic_control
      );
      this.projectDetailsForm.value.start_date =
        this.projectDetailsForm.value.start_date;
      this.projectDetailsForm.value.end_date =
        this.projectDetailsForm.value.end_date;
  ...
  getCurrentUser() {
    ...
    if (application.applicant_details) {
      this.applicantForm.controls.applicant_name.setValue(
        application.applicant_details.applicant_name
      );
      this.applicantForm.controls.applican_last_name.setValue(
        application.applicant_details.applican_last_name
      );

      this.applicantForm.controls.applicant_email.setValue(
        application.applicant_details.applicant_email
      );
      this.applicantForm.controls.applicant_business.setValue(
        application.applicant_details.applicant_business
      );
      this.applicantForm.controls.applicant_address.setValue(
        application.applicant_details.applicant_address
      );
      this.applicantForm.controls.applicant_phone.setValue(
        application.applicant_details.applicant_phone
      );
      this.applicantForm.controls.applicant_city.setValue(
        application.applicant_details.applicant_city
      );
      if (application.applicant_details?.applicant_state) {
        this.applicantForm.controls.applicant_state.setValue(
          application.applicant_details.applicant_state
        );
      }
      this.applicantForm.controls.applicant_zip.setValue(
        application.applicant_details.applicant_zip
      );
    } else {
      this.authService.getUserInfo().subscribe((currentUser) => {
        ...
        if (this.currentUserInfo) {
          this.applicantForm.controls.applicant_name.setValue(
            this.currentUserInfo.first_name
          );
          this.applicantForm.controls.applican_last_name.setValue(
            this.currentUserInfo.last_name
          );
          ...
          this.applicantForm.controls.applicant_business.setValue(
            this.currentUserInfo.company
          );
          this.applicantForm.controls.applicant_address.setValue(
            this.currentUserInfo.address
          );
          this.applicantForm.controls.applicant_phone.setValue(
            this.currentUserInfo.phone_number
          );
          this.applicantForm.controls.applicant_city.setValue(
            this.currentUserInfo.city
          );
          this.applicantForm.controls.applicant_state.setValue(
            this.currentUserInfo.state
          );
          this.applicantForm.controls.applicant_zip.setValue(
            this.currentUserInfo.zip
          );
        }
      });
    }
  ...
  whereTab() {
    ...
    if (application.location_type == 1) {
      ...
      application.location_type == 2 &&
      application.location.length > 0
    ) {
      if (application.location.length > 1) {
        for (let index = 0; index < application.location.length - 1; index++) {
          if (
            application.location.length != this.addLocationControls.value.length
          ) {
            this.addLocationControls.push(this.addLocationFormGroup());
          }
  ...
  contractorTab() {
    ...
    if (
      ...
      this.application.role == 1 &&
      !this.application.contractor_details
    ) {
    ...
    if (
      ...
      this.contractorForm.controls.contractor_for_job.setValue(1);
      this.contractorForm.controls.contractor_name.setValue(
        this.currentUserInfo.first_name + " " + this.currentUserInfo.last_name
      );
      this.contractorForm.controls.contractor_email.setValue(
        this.currentUserInfo.email
      );
      this.contractorForm.controls.contractor_business.setValue(
        this.currentUserInfo.company
      );
      this.contractorForm.controls.contractor_address.setValue(
        this.currentUserInfo.address
      );
      this.contractorForm.controls.contractor_phone.setValue(
        this.currentUserInfo.phone_number
      );
      this.contractorForm.controls.contractor_city.setValue(
        this.currentUserInfo.city
      );
      this.contractorForm.controls.contractor_state.setValue(
        this.currentUserInfo.state
      );
      this.contractorForm.controls.contractor_zip.setValue(
        this.currentUserInfo.zip
      );
    } else {
      const application = this.permitService.getApplication();
      if (application.contractor_details) {
        this.isDisabled = false;
        this.contractorForm.controls.contractor_for_job.setValue(
          application.contractor_details.contractor_for_job
        );
        this.contractorForm.controls.contractor_name.setValue(
          application.contractor_details.contractor_name
        );
        this.contractorForm.controls.contractor_email.setValue(
          application.contractor_details.contractor_email
        );
        this.contractorForm.controls.contractor_business.setValue(
          application.contractor_details.contractor_business
        );
        this.contractorForm.controls.contractor_address.setValue(
          application.contractor_details.contractor_address
        );
        this.contractorForm.controls.contractor_phone.setValue(
          application.contractor_details.contractor_phone
        );
        this.contractorForm.controls.contractor_city.setValue(
          application.contractor_details.contractor_city
        );
        this.contractorForm.controls.contractor_state.setValue(
          application.contractor_details.contractor_state
        );
        this.contractorForm.controls.contractor_zip.setValue(
          application.contractor_details.contractor_zip
        );
      }
    }
  }
  ...
  fillDetails() {
    ...
    if (this.contractorForm.value.contractor_for_job == 2) {
      ...
      this.contractorForm.controls.contractor_email.setValue(
        contractor && contractor.contractor_email
          ? contractor.contractor_email
          : null
      );
      this.contractorForm.controls.contractor_business.setValue(
        contractor && contractor.contractor_business
          ? contractor.contractor_business
          : null
      );
      this.contractorForm.controls.contractor_address.setValue(
        contractor && contractor.contractor_address
          ? contractor.contractor_address
          : null
      );
      this.contractorForm.controls.contractor_phone.setValue(
        contractor && contractor.contractor_phone
          ? contractor.contractor_phone
          : null
      );
      this.contractorForm.controls.contractor_city.setValue(
        contractor && contractor.contractor_city
          ? contractor.contractor_city
          : null
      );
      this.contractorForm.controls.contractor_state.setValue(
        contractor && contractor.contractor_state
          ? contractor.contractor_state
          : null
      );
      this.contractorForm.controls.contractor_zip.setValue(
        contractor && contractor.contractor_zip
  ...
  projectDetailsTab() {
    ...
    if (application.project_detail) {
      this.projectDetailsForm.controls.purpose.setValue(
        application.project_detail.purpose
      ),
        this.projectDetailsForm.controls.dig_safely_no.setValue(
          application.project_detail.dig_safely_no
        ),
        this.projectDetailsForm.controls.traffic_control.setValue(
          application.project_detail.traffic_control
        ),
        this.projectDetailsForm.controls.length.setValue(
          application.project_detail.length
        ),
        this.projectDetailsForm.controls.width.setValue(
          application.project_detail.width
        ),
        this.projectDetailsForm.controls.depth.setValue(
          application.project_detail.depth
        ),
        this.projectDetailsForm.controls.opening_size.setValue(
          application.project_detail.opening_size
        ),
        this.projectDetailsForm.controls.opening_number.setValue(
          application.project_detail.opening_number
        ),
        this.projectDetailsForm.controls.pavement_type.setValue(
          application.project_detail.pavement_type
        ),
        this.projectDetailsForm.controls.start_date.setValue(
          new Date(application.project_detail.start_date)
        ),
        this.projectDetailsForm.controls.end_date.setValue(
          new Date(application.project_detail.end_date)
        ),
        this.projectDetailsForm.controls.description.setValue(
          application.project_detail.description
        );
    }
  }
  ...
  submitApplicationAfterReview() {
    if (this.submitApplicationValue.length == 7) {
      ...
      this.toasterService.error(
        "Please review all tab then submit application"
      );
    }
  }
  ...
  submitApplications() {
    this.permitService
      .submitAppliction({ application_id: this.application.id })
      .subscribe((data) => {
        //  this.router.navigate(['/dashboard/submit-application'], { queryParams: { id: this.application.id } })
        this.router.navigate(["/dashboard/payment"], {
          queryParams: { id: this.application.id, fee_type: 3 },
        });
      });
  }
  ...
  updateAppliction() {
    this.permitService
      .updateApplication(this.application_id)
      .subscribe((data) => {
  ...
  addDuplimester() {
    ...
    this.duplimesterForm.value.dumpster_state = Number(
      this.duplimesterForm.value.dumpster_state
    );

    this.permitService
      .addDuplimester(this.duplimesterForm.value)
      .subscribe((data) => {
    ...
    this.permitService.exextAddress(data).subscribe((data) => {
      ...
      if (this.exactAddress.length > 0) {
        this.selectadd = this.exactAddress.map((data) => {
          const address =
            data.property_location.split(",")[0] + ", " + data.state;
          return address;
  ...
  phoneNumberFormate(value: string) {
    ...
    if (value == "app") {
      if (
        this.applicantForm.value.applicant_phone &&
        this.applicantForm.value.applicant_phone.length === 3
      ) {
        this.applicantForm.controls.applicant_phone.setValue(
          this.applicantForm.value.applicant_phone.concat(autoFillValue)
        );
      }
      if (
        this.applicantForm.value.applicant_phone &&
        this.applicantForm.value.applicant_phone.length === 7
      ) {
        this.applicantForm.controls.applicant_phone.setValue(
          this.applicantForm.value.applicant_phone.concat(autoFillValue)
        );
      ...
      if (
        this.contractorForm.value.contractor_phone &&
        this.contractorForm.value.contractor_phone.length === 3
      ) {
        this.contractorForm.controls.contractor_phone.setValue(
          this.contractorForm.value.contractor_phone.concat(autoFillValue)
        );
      }
      if (
        this.contractorForm.value.contractor_phone &&
        this.contractorForm.value.contractor_phone.length === 7
      ) {
        this.contractorForm.controls.contractor_phone.setValue(
          this.contractorForm.value.contractor_phone.concat(autoFillValue)
        );
      ...
      if (
        this.duplimesterForm.value.dumpster_phone &&
        this.duplimesterForm.value.dumpster_phone.length === 3
      ) {
        this.duplimesterForm.controls.dumpster_phone.setValue(
          this.duplimesterForm.value.dumpster_phone.concat(autoFillValue)
        );
      }
      if (
        this.duplimesterForm.value.dumpster_phone &&
        this.duplimesterForm.value.dumpster_phone.length === 7
      ) {
        this.duplimesterForm.controls.dumpster_phone.setValue(
          this.duplimesterForm.value.dumpster_phone.concat(autoFillValue)
        );
